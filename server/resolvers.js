...
const resolvers = {
  Query: {
      ...
      chosenState: (parent, args, context, info) => {
        const { states } = args;
        let park = parks.filter( park => park.states.includes(states) );
        return park;
    },
      chosenRegion: (parent, args, context, info) => {
        const { regions } = args;
        let park = parks.filter( park => park.regions.includes(regions) );
        return park;
    }
  }
};
